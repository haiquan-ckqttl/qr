/* ============================================================
   1. CẤU HÌNH VỊ TRÍ CỘT EXCEL (INDEX MAPPING)
   ------------------------------------------------------------
...
/* ============================================================
   2. QR GENERATOR FALLBACK (JS THUẦN)
   ============================================================ */
...
/* ============================================================
   3. TRẠNG THÁI & KHỞI TẠO HỆ THỐNG
   ============================================================ */
var phieus = [];
var nextId = 1;
...
/* ============================================================
   4. QUẢN LÝ DANH SÁCH PHIẾU (CRUD)
   ============================================================ */
function addPhieu(){
    var id = nextId++;
        ...
        +'<div class="card-body" id="body-'+p.id+'">'
            +'<div class="form-grid">'
                +'<div class="fg">'
                    ...
                    +'<div class="fi-row">'
                        +'<input class="fi" id="f'+p.id+'mst" placeholder="VD: 4800185029" oninput="setField('+p.id+',\'mst\',this.value)">'
                ...
                +'<div class="fg col-span-2">'
                    ...
                    +'<input class="fi" id="f'+p.id+'tencty" placeholder="VD: Công ty ......" oninput="setField('+p.id+',\'tencty\',this.value)">'
                +'</div>'
                +'<div class="fg">'
                ...
                +'<div class="fg">'
                    ...
                    +'<input class="fi" id="f'+p.id+'nguoikhai" placeholder="VD: Nguyễn Văn A" oninput="setField('+p.id+',\'nguoikhai\',this.value)">'
                +'</div>'
                +'<div class="fg">'
...
/* ============================================================
   6. KIỂM TRA ĐIỀU KIỆN & VALIDATION
   ============================================================ */
...
function doPrint(){
    ...
    Promise.all(list.map(function(p){
        ...
        if(paper === 'a5'){
            style.textContent = '@media print{ @page{size:A5 landscape; margin:6mm;} ' + commonPrintCSS + ' }';
        } else {
            style.textContent = '@media print{ @page{size:A4 portrait; margin:8mm 6mm;} ' + commonPrintCSS + ' }';
        }
...
function buildPrintHTML(items, paper){
    ...
    // Tạo chuỗi ngày tự động điền (DD/MM/YYYY)
    var currentDateStr = pad(now.getDate()) + '/' + pad(now.getMonth() + 1) + '/' + now.getFullYear();
    ...
    function ticketHTML(item){
        ...
        return '<div style="font-family:\'Times New Roman\',Times,serif; color:#000; background:#fff;">'
            // KHUNG CÓ VIỀN
            +'<div class="ticket-double" style="display:flex; border:1.5px solid #000; width:100%; box-sizing:border-box;">'
                // PHIẾU HẢI QUAN (TRÁI)
                +'<div class="ticket-side" style="flex: 7; padding:2mm 3mm; padding-right:6mm; border-right:1.5px dashed #000; display:flex; flex-direction:column; min-width:0;">'
                    +'<div style="display:flex; align-items:center; justify-content:center; gap: 12px; border-bottom: 1.5px solid #000; padding-bottom: 1.5mm; margin-bottom: 1.5mm;">'
                        +'<img src="./images/logo-hq.png" style="width:38px; height:38px; object-fit:contain;" onerror="this.style.display=\'none\'"/>'
                        +'<div style="text-align:left; font-size:8.5pt; font-weight:bold; text-transform:uppercase; line-height:1.35;">CHI CỤC HẢI QUAN KHU VỰC XVI<br><span style="font-size:9pt;">HẢI QUAN CỬA KHẨU QUỐC TẾ TÀ LÙNG</span></div>'
                    +'</div>'
                    +'<div style="display:flex; justify-content:space-between; font-size:8pt; margin-bottom:1mm;">'
                        +'<span>Số: ....................</span>'
                        +'<span>Ngày: ' + currentDateStr + '</span>' // Điền ngày tự động
                    +'</div>'
                    +'<div style="text-align:center; font-size:9pt; font-weight:bold; text-transform:uppercase; border-top:1px solid #000; border-bottom:1px solid #000; padding:1mm 0; margin-bottom:1.5mm;">PHIẾU ĐĂNG KÝ PTVT CHỞ HÀNG XUẤT KHẨU (HQ)</div>'
                    +'<div style="flex:1; min-width:0;">'
                        +'<div class="tf" style="padding:1.2mm 0;"><span class="tf-l">1. Số tờ khai:</span><span class="tf-v"><b>'+p.sotokhai+'</b></span></div>'
                        +'<div class="tf" style="padding:1.2mm 0;"><span class="tf-l">2. Biển kiểm soát:</span><span class="tf-v"><b>'+p.bks+'</b></span></div>'
                        ...
                        +'<div class="tf" style="padding:1.2mm 0;"><span class="tf-l">6. Doanh nghiệp:</span><span class="tf-v">'+p.nguoikhai+'</span></div>'
                ...
                +'<div class="ticket-side" style="flex: 3; padding:2mm 3mm; padding-left:6mm; display:flex; flex-direction:column; min-width:0;">'
                    ...
                    +'<div style="font-size: 8pt; display:flex; flex-direction:column; gap:1.5mm;">'
                        ...
                        +'<div><b style="font-size:7.5pt;">3. NGÀY XC:</b><div style="height:5mm; border-bottom:1px dotted #aaa;"></div></div>'
                        +'<div><b style="font-size:7.5pt;">4. NGÀY NC:</b><div style="height:5mm; border-bottom:1px dotted #aaa;"></div></div>'
                    +'</div>'
                    +'<div style="margin: 2mm auto 1mm; width:100%; display:flex; flex-direction:column; align-items:center;">'
                        +(qrURL
                        ? '<img src="'+qrURL+'" style="width:22mm;height:22mm;display:block;"/>'
                        : '<div style="width:22mm;height:22mm;border:1px solid #ccc;display:flex;align-items:center;justify-content:center;"><span style="font-size:7pt;color:#aaa;">QR</span></div>'
                        )
                        +'<div style="font-size: 6pt; font-weight:bold; text-transform:uppercase; margin-top:1mm;">Mã quét Excel</div>'
                    +'</div>'
                +'</div>'
            +'</div>'
            
            // CHÂN KÝ & ĐÓNG DẤU NẰM NGOÀI VIỀN
            +'<div style="display:flex; margin-top: 2mm;">'
                +'<div style="flex: 7; text-align:center;">'
                    +'<b style="font-size:8.5pt;">CC Giám sát</b><br>'
                    // +'<i style="font-size:7.5pt; color:#444;">(Ký, đóng dấu)</i>'
                    +'<div style="height: 12mm;"></div>'
                +'</div>'
                +'<div style="flex: 3; text-align:center;">'
                    +'<b style="font-size:8pt;">Người khai hải quan</b><br>'
                    // +'<i style="font-size:7pt; color:#444;">(Ký, ghi rõ họ tên)</i>'
                    +'<div style="height: 12mm;"></div>'
                    +'<b style="font-size:8.5pt;">'+(p.nguoikhai || '......................')+'</b>'
                +'</div>'
            +'</div>'
            
            // FOOTER NGÀY GIỜ IN
            +'<div style="margin-top:1mm; margin-right:2mm; padding-top:1mm; border-top:.5px solid #bbb; font-size:7.5pt; color:#333; font-style:italic; font-family:sans-serif; display:flex; justify-content:space-between;">'
                +'<span></span>'
                +'<span>In: '+ts+'</span>'
            +'</div>'
        +'</div>';
